...
void addNode()
{
    ...
    if (START == NULL || newNode->noMhs <= START->noMhs)
    {
        ...
        // jika list ksosong, maka node nextnya adalah START
    ...
    else
    {
        ...
        if (previous != NULL)
        {
            previous->next = newNode; // step 7: Make the next of the previous
        }
...
void deleteNode()
{
    ...
    if (current = START)
    {
        START = START->next; // step 2: update the START pinter
        if (START != NULL)
        {
            START->prev = NULL;
        }
...
// prosedur untuk mencari data dan menampikan data yang dicari jika ada
void searchData()
{
    if (listEmpty() == true)
    {
